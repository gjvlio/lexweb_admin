  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-5 pb-0">
        <p className="text-[10px] font-semibold font-sans uppercase tracking-widest text-black mb-1.5">
          Plan Name
        </p>
        {/* Boxed read-only display — styled to match the image's input look */}
        <div className="border border-slate-300 rounded-md px-3 py-2 bg-white text-sm font-sans text-black select-none truncate">
          {plan.name}
        </div>
      </div>
      ...
      <div className="px-5 pt-3 pb-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="border border-slate-300 rounded-md px-3 py-2 bg-white text-sm font-sans text-black select-none min-w-0 flex-1 truncate">
            {plan.price === 0 || plan.price === 'Free' ? 'Free' : Number(plan.price).toLocaleString()}
          </div>
          <span className="text-xs font-sans text-black whitespace-nowrap shrink-0">
            /month
          </span>
        </div>
      </div>
